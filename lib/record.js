...
module.exports = (bufferSerializer, config, encryption, fsAsync, OtDate, promise, zlibAsync) => {
     ...
     * @param {*} [metadata] Stored outside the inner encryption for later access
     * @return {Promise.<Buffer>}
     */
    function freezeAsync(data, innerKey, options, metadata) {
        var dataBuffer, expires;

        options = options || {};
        expires = OtDate.now().plus(config.record.lifetimeMaximum);
        ...
        return zlibAsync.deflateRawAsync(dataBuffer).then((deflated) => {
            ...
            if (metadata) {
                toBeSerialized.meta = metadata;
            }

            buffer = bufferSerializer.toBuffer(toBeSerialized);
