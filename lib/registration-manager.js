...
module.exports = (accountManager, config, email, promise, random, storageServiceFactory, totp) => {
    ...
    function secureInfo(registrationId, record) {
        ...
        if (!record.totpConfirmed) {
            response.secureInfo.totp = record.totp;
        }
    ...
    function confirmEmailAsync(registrationId, confirmationCode) {
        return storageService.getAsync(registrationId).then((record) => {
            if (!record.passwordHash || !record.totpConfirmed) {
    ...
    function secureAsync(registrationId, request, server) {
        return storageService.getAsync(registrationId).then((record) => {
            if (!totp.verifyCurrentAndPrevious(record.totp.key, request.totp.current, request.totp.previous)) {
                throw new Error("TOTP validation failed");
            }

            record.passwordHash = request.passwordHash;
            record.totpConfirmed = true;
