...
module.exports = () => {
    ...
    /**
     * Decodes data that was encoded as Base64 for URIs.  When provided
     * a string, returns a string.  When given a Buffer, returns a Buffer.
     *
     * @param {(Buffer|string)} data
     * @return {(Buffer|string)} decoded
     */
    function decodeForUri(data) {
        ...
        if (typeof data === "string") {
            ...
            copy = data.toString("binary");
        }

        copy = copy.replace(/-/g, "+").replace(/_/g, "/");
