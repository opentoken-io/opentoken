...
module.exports = (config, random, storageServiceFactory) => {
    var challengeHashConfig, challengeIdLength, storageService;
    ...
    function confirmAsync(accountId, passwordHash, challengeHash) {
        ...
        return loadListAsync(accountId).filter((challengeId) => {
            ...
            return hash.hashAsync(passwordHash + challengeId, challengeHashConfig).then((result) {
                ...
                throw new Exception("Did not match any known challenge result");
            });
        });
    }
    ...
    function createAsync(accountId) {
        return promise.props({
            ...
            return saveListAsync(storageKey, list).then(() => {
                return {
    ...
    storageService = storageServiceFactory(config.challenge.idHash, config.challenge.lifetime, config.challenge.storagePrefix);
