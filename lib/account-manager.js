...
module.exports = (base64, challengeManager, config, OtDate, promise, random, record, sessionManager, storageServiceFactory, totp) => {
    ...
    function loginAsync(id, loginRequest) {
        ...
        return storageService.getAsync(id).then((recordData) => {
            return challengeManager.validateAsync(id, recordData.passwordHash, loginRequest.challengeHash).then(() => {
                if (!totp.verifyCurrent(recordData.mfa.totp.key, loginRequest.mfa.totp)) {
                    throw new Error("MFA TOTP did not validate");
                }

                return sessionManager.createAsync(id).then((sessionId) => {
                    return {
                        sessionId
                    };
                });
    ...
    function loginHashConfigAsync(id) {
        ...
        // simultaneously.
        return storageService.getAsync(id).then((recordData) => {
            return challengeManager.createAsync(id).then((challengeConfig) => {
                return {
