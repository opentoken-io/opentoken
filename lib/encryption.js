...
module.exports = (ciphersAndHashes, cryptoAsync, promise, random) => {
    ...
    function decryptAsync(data, keySource) {
        ...
        if (typeof keySource === "string") {
            keySource = Buffer.from(keySource, "binary");
        }
        ...
        return readHeaderAsync(data).then((header) => {
            ...
            return promise.props({
                cipherKey: cryptoAsync.pbkdf2Async(keySource, Buffer.alloc(0), parameters.cipherKey.iterations, parameters.cipherKey.bytes, parameters.cipherKey.name),
                hmacKey: cryptoAsync.pbkdf2Async(keySource, Buffer.alloc(0), parameters.hmacKey.iterations, parameters.hmacKey.bytes, parameters.hmacKey.name)
            }).then((bits) => {
