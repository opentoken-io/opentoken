 ...
 *
 * @typedef {AccountService~accountFile}
 * @param {string} accountId
 * @param {string} email
 * @param {string} mfaKey
...
class AccountService {
    ...
    constructor(config, secureHash, Storage) {
        ...
        this.storage = Storage.configure(this.config.storage);
    }
    ...
    /**
     * Gets the directory where the account will be/is stored. This takes options
     * passed in from the manager to direct where to get/put account information.
     *
     * @param {string} accountId
     * @param {Object} config
     * @return {Promise.<string>}
     */
    getDirectoryAsync(accountId, config) {
        config = config || {};

        return this.secureHash.hashAsync(accountId, config.idHash || {}).then((hashedAccountId) => {
            return (config.accountDir || "account/") + hashedAccountId;
        });
    }
