...
module.exports = function (config, promise, secureHash, storage) {
    ...
    function deleteLoginFileAsync(hashedAccountId, loginId) {
        return secureHash.secureHashEncodedUriAsync(loginId, accountConfig.idHash).then((hashedLoginId) => {
            var loginKey;
    ...
    function hashPasswordAsync(accountId, salt) {
        return getDirectoryAsync(accountId).then((accountFile) => {
            return secureHash.secureHashEncodedAsync(accountFile.password + salt, accountConfig.secureHash).then((hashedPassword) => {
                return secureHash.createHash(hashedPassword, accountConfig.challenge.algo);
            });
        });
    }
    ...
    function getDirectoryAsync(prefix, idToHash) {
        return secureHash.secureHashEncodedUriAsync(idToHash, accountConfig.idHash).then((hashedId) => {
    ...
    function makeLoginKey(accountIdHash, loginIdHash) {
        if (!accountConfig.accountDir || !accountConfig.loginDir) {
            throw new Error("Missing configuration options to create login key");
        }
        return accountConfig.accountDir + accountIdHash + accountConfig.loginDir + loginIdHash;
    }
    ...
    function putLoginFileAsync(loginFile, hashedAccountId, options) {
        return secureHash.secureHashEncodedUriAsync(loginFile.loginId, accountConfig.idHash).then((loginIdHash) => {
            var loginKey;
