...
module.exports = function(config, secureHash, Storage) {
    ...
    function completeAsync (accountInfo, options, regId) {
        return getRegistrationFileAsync(regId).then((regInfo) => {
            regInfo.accountFile.accountId = accountInfo.accountId;
            regInfo.accountFile.password = accountInfo.password;
            delete regInfo.accountFile.regId;

            return makeAccountFileAsync(regInfo.accountFile, options).then(() => {
                return storage.delAsync(regInfo.regDir).then(() => {
                    return {
