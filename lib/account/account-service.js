...
module.exports = function (config, secureHash, Storage) {  // I would like to use Storage, because that's what we have in the dependencies and it would keep that link in mind.
    ...
    function completeAsync(accountInfo, options, regId) {
        return getDirectoryAsync(registrationDirName, regId).then(getAsync).then((accountFile) => {
            ...
            return makeAccountFileAsync(accountFile, options).then(storage.delAsync);
        }).then(() => {
            return {
    ...
    function makeAccountFileAsync(accountInfo, options) {
        return getDirectoryAsync(accountDirName, accountInfo.accountId).then((accountDir) => {
            return storage.putAsync(accountDir, JSON.stringify(accountInfo), options).then(() => {
                return accountDir;
            });
        });
    }
