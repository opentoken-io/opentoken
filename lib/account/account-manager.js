...
module.exports = function (accountService, base64, validatedConfig, hotp, otDate, promise, random, secureHash) {
    var accountConfig;

    accountConfig = validatedConfig.account || {};
    ...
    function loginInitiateDetails(details, salt) {
        return accountService.getAccountFileAsync(details.accountId).then((accountFile) => {
            return random.randomIdAsync(accountConfig.passwordSaltLength).then((passwordSaltId) => {
                return {
                    pbkdf2: accountConfig.secureHash,
                    challenge: {
                        salt: salt,
                        hash: accountConfig.challenge.algo
                    },
    ...
    function loginInitiationAsync(hashedAccountId, loginInitiateInfo) {
        return secureHash.secureHashEncodedUriAsync(loginInitiateInfo.accountId, accountConfig.idHash).then((accountIdHash) => {
            if (! secureHash.compare(accountIdHash, hashedAccountId)) {
                throw new Error("Account hashes do not match");
            }

            return promise.props({
                loginId: random.randomIdAsync(accountConfig.loginIdLength),
                salt: random.randomIdAsync(accountConfig.challenge.saltLength)
            }).then((bits) => {
    ...
    function signupCompleteAsync(registrationInfo) {
        return accountService.getRegistrationFileAsync(registrationInfo.regId).then((accountFile) => {
            ...
            return random.randomIdAsync(accountConfig.accountIdLength);
        }).then((accountId) => {
    ...
    function signupInitiationAsync(initiationInfo) {
        return promise.props({
            ...
            regId: random.randomIdAsync(accountConfig.registrationIdLength),
            passwordSalt: random.randomIdAsync(accountConfig.passwordSaltLength)
        }).then((bits) => {
