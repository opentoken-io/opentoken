...
module.exports = function (accountService, config, hotp, otDate, promise, random) {
    ...
    function signupCompleteAsync(accountInfo) {
        return accountService.getRegistrationFileAsync(accountInfo.regId).then((accountFile) => {
            ...
            return random.randomizedAsync(config.account.accountIdLength || 24);
        }).then((accountId) => {
    ...
    function signupInitiationAsync(accountInfo) {
        return promise.props({
            ...
            regId: random.randomizedAsync(config.account.registrationIdLength || 24),
            passwordSalt: random.randomizedAsync(config.account.passwordSaltLength || 128)
        }).then((bits) => {
            var options;
