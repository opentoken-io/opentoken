...
module.exports = function (accountService, config, hotp, otDate, promise, random, secureHash) {
    ...
    function loginCompleteAsync(hashedAccountId, loginId, loginInfo) {
        return accountService.getLoginFileAsync(hashedAccountId, loginId).then((loginFile) => {
            return accountService.getAccountFileAsync(loginFile.accountId).then((accountFile) => {
                return loginVerifyAsync(accountFile, loginFile.challengeId, loginFile.salt, loginInfo.password);
            }).then(() => {
                checkMfa(loginInfo.currentMfa);
     ...
     * Used three levels of hashing to create the super hashed password
     *
     ...
     */
    function hashPasswordAsync(password, challenge, salt) {
        var hashLevels, primary;

        hashLevels = accountConfig.passwordHash;
        primary = hashLevels.primary;
        primary.salt = salt;

        return secureHash.hashAsync(password, primary, true).then((passSaltHash) => {
            return secureHash.hashAsync(passSaltHash + challenge, hashLevels.secondary, true).then((passSaltChallengeHash) => {
                return secureHash.hashAsync(passSaltChallengeHash, hashLevels.tertiary, true);
            });
        });
    }
     ...
     */
    function loginVerifyAsync(accountFile, loginId, salt, hashedPassword) {
        return hashPasswordAsync(accountFile.password, loginId, salt).then((passwordHash) => {
            if (! secureHash.compare(passwordHash, hashedPassword)) {
