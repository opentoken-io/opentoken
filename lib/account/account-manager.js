...
/**
 * Collection of information stored containing
 * information about a particular account holder.
 *
...
/**
 * Challenge information sent to the account holder to
 * create the same secure hash the system created
 * when account holder initiated logging into the system.
 *
...
module.exports = function (accountService, config, hotp, otDate, promise, random, secureHash) {
    ...
    function loginCompleteAsync(hashedAccountId, loginId, loginInfo) {
        return accountService.getLoginFileAsync(hashedAccountId, loginId).then((loginFile) => {
            if (! secureHash.compare(loginFile.secureHash, loginInfo.secureHash)) {
