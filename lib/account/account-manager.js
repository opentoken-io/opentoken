...
module.exports = function (accountService, config, hotp, otDate, promise, random, secureHash) {
    ...
    function hashPasswordAsync(password, challenge, salt) {
        var hashLevels, secondary;

        hashLevels = accountConfig.passwordHash;
        secondary = hashLevels.secondary;
        secondary.salt = salt;

        return secureHash.hashAsync(password, secondary, true).then((passSaltHash) => {
            return secureHash.hashAsync(passSaltHash + challenge, hashLevels.secondary, true);
        });
    }
    ...
    function signupConfirmAsync(regId) {
        return accountService.getRegistrationFileAsync(regId).then((accountFile) => {
            return {
                ...
                pbkdf: accountConfig.passwordHash.primary,
                encoding: "base64"
            };
        });
    }
