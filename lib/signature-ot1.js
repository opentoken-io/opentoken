...
module.exports = (accessCodeManager, errorResponse, hash, promise, util) => {
     ...
     *
     * @param {validateSignatureMiddleware~signatureDescription} desc
     * @return {Promise.<*>}
     */
    function checkSignatureDescriptionAsync(desc) {
        // We can skip type and signature.  Only focus on these three
        // attributes.
        if (desc.method !== "HMAC") {
            return errorResponse.rejectedPromiseAsync(`Prohibited method, only HMAC allowed: ${desc.method}`, "X3A8VmGT");
        }

        if (desc.algorithm !== "SHA256") {
            return errorResponse.rejectedPromiseAsync(`Prohibited algorithm, only SHA256 allowed: ${desc.algorithm}`, "QpEMHUg5");
        }

        if (desc.encoding !== "HEX") {
            return errorResponse.rejectedPromiseAsync(`Prohibited encoding, only HEX allowed: ${desc.encoding}`, "B7q03SsU");
        }
