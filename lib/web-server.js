...
class WebServer {
    constructor(fs, helmet, logger, proxywrap, restify) {
    ...
    app() {
        if (! this.server) {
            ...
            // Add helmet for security-related modules
            this.server.use(this.helmet.xssFilter());
            this.server.use(this.helmet.frameguard({
                action: "deny"
            }));

            if (this.config.https) {
                this.server.use(this.helmet.hsts({
            ...
            this.server.use(this.helmet.hidePoweredBy());
            this.server.use(this.helmet.ieNoOpen());
            this.server.use(this.helmet.noSniff());
            this.server.use(this.helmet.noCache());
        }
