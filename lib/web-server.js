 ...
 * @param {opentoken~formatters} formatters
 * @param {opentoken~fsAsync} fsAsync
 * @param {opentoken~logger} logger
 * @param {opentoken~MiddlewareProfiler} MiddlewareProfiler
 * @param {opentoken~promise} promise
 * @param {opentoken~random} random
 * @param {Restify} restify
 * @param {opentoken~restifyRouterMagicAsync} restifyRouterMagicAsync
 * @param {opentoken~restMiddleware} restMiddleware
 * @return {opentoken~WebServer}
 */
module.exports = (container, ErrorResponse, formatters, fsAsync, logger, MiddlewareProfiler, promise, random, restify, restifyRouterMagicAsync, restMiddleware) => {
    /**
     * A configured web server, able to serve files from a configurable route
     * folder.
     *
     * @class {opentoken~WebServer}
     */
    class WebServer {
        /**
        ...
        app() {
            ...
            if (this.config.https) {
                promiseResult = promise.all([
                    fsAsync.readFileAsync(this.config.certificateFile).then((contents) => {
                        this.restifyConfig.certificate = contents;
                    }),
                    fsAsync.readFileAsync(this.config.keyFile).then((contents) => {
         ...
         * types, suppresses their output to console or to the client and then
         * they get caught by the Uncaught Exception Handler.
         *
         * @private
         * @return {Function}
         */
        attachErrorHandlers() {
            return (server) => {
                server.on("uncaughtException", (req, res, route, exception) => {
                    var error;

                    req.log(`Uncaught Exception: ${exception.toString()}`);
                    logger.console("Uncaught Exception:", exception, exception.stack);

                    error = new ErrorResponse("Uncaught Exception", "tD2G3B7o");
                    res.contentType = error.mimeType();
                    res.send(500, error);
                });
                server.on("restifyError", (req, res, error, next) => {
                    // Can really only log or mutate the object that
                    // is passed in.  The formatters will take care of
                    // Error objects nicely.
                    req.log(`Exception: ${error.toString()}`);
                    logger.console(error.stack);
                    next();
                });
        ...
        configure(config) {
            ...
            this.restifyConfig = {
                // This is all pulled from another module
                formatters,
        ...
        profileMiddleware() {
            return (server) => {
                ...
                profiler.displayAtInterval(logger.info.bind(logger), 10000);
