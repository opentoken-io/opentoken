...
module.exports = (container, fsAsync, logger, MiddlewareProfiler, promise, random, restify, restifyRouterMagicAsync, restMiddleware) => {
    /**
    ...
    class WebServer {
        ...
        attachErrorHandlers() {
            return (server) => {
                server.on("uncaughtException", (req, res, route, error) => {
                    ...
                    logger.console("Uncaught Exception:", error, error.stack);
                    res.send(500);
                });
                server.on("restifyError", (req, res, error, next) => {
                    random.idAsync(this.config.exceptionIdLength).then((logref) => {
                        // This automatically is already sending a 500 status code
                        logger.error(`Exception [${logref}]: ${error.toString()} ${error.stack}`);
                        logger.console("Exception:", error);
                        res.contentType = "application/vnd.error+json";

                        // Assign values to the object, blindly overwriting
                        // what's there.  Only these properties are reported
                        // using the formatter.
                        error.message = "Exception thrown during processing of request";
                        error.logref = logref;
                        next();
                    });
                });
        ...
        configure(config) {
            ...
            this.config = {
                ...
                exceptionIdLength: config.exceptionIdLength,
            ...
            this.restifyConfig = {
                formatters: {
                    // Errors
                    "application/vnd.error+json; q=0.1": (req, res, body, next) => {
                        var copy;

                        // Sanitize and also be allowed to log the "message"
                        // property.  When body is an Error object,
                        // JSON.stringify doesn't capture the message
                        // property.
                        copy = {
                            logref: body.logref,
                        ...
                        return next(null, JSON.stringify(copy));
                    },

                    // Display PNGs from the API
